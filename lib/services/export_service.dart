...
class ExportService {
  ...
  /// in the summary.
  Future<({String name, Uint8List bytes})> buildExcelReport(
    ...
    final name =
        'expiry_report_${slug}_${_stampFmt.format(DateTime.now())}.xlsx';
  ...
  Future<({String name, Uint8List bytes})> buildJsonBackup(
      List<Product> products, List<Store> stores) async {
    final name = 'expiry_backup_${_stampFmt.format(DateTime.now())}.json';
    final payload = {
      'app': 'expiry_check',
      'version': 2,
